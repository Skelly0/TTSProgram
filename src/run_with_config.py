...
def load_config():
    ...
    try:
        ...
        # Remove the config directory from the system path
        if sys.path[0].endswith('config'):
            sys.path.pop(0)
...
def main():
    ...
    try:
        if config.ENABLE_LLM_PREPROCESSING:
            ...
            preprocess_cmd = [
                "python", os.path.join("src", "preprocess_documents.py"),
        ...
        tts_cmd = [
            "python", os.path.join("src", "run_tts.py"),
